<script>
    ...
    const links = [['About','#about'],['Portfolio','#portfolio'],['Contact','#contact'],['Resume','#resume'],['Github','#github']]
...
<nav id="navi" class="{`navbar is-fixed-top ${postHero?'':'hide'}`}" >
    <div class="navbar-brand m-0">
        ...
        <div class="{`navbar-burger ${active?'is-active':''}`}" on:click={activate}>
    ...
    <div class="{`navbar-menu m-0 ${active?'is-active':''}`}">
        ...
        <div class="navbar-end m-0">
            {#each links as link}
                <a href={link[1]} on:click={activate} class="navbar-item is-tab is-spaced">{link[0]}</a>
            {/each}
        </div>
    </div>
</nav>
